...
//nCr function
ll nCr(int n,int r){ //complexity O(n)
    ll ans= fact[n];
    ans = ans* factinv[r]%mod; // ans= (n!)/((r)!*(n-r)!) so n! devided by r! whichh means n! * inverse of r! and inverse of r!=  modexpo(fact[r], mod-2,mod) and mode to keep things small
    ans = ans* factinv[n-r]%mod; // this is for detecting (n-r)!'s reverse and multiplying with others and then mode to keep things small
    return ans;
}
int main(){
    ...
    cout<<a<<"C"<<b<<": "<<nCr(a,b)<<endl;// calling the nCr function
